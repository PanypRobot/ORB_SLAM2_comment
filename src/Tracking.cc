...
{
    // Step 1 从配置文件中加载相机参数
    // cv::FileStorage提供函数供序列化和反序列化, 从yaml配置文件中读取出配置参数
    ...
    DistCoef.copyTo(mDistCoef); //mDistCoef是相机的去畸变矩阵
    ...
    // Step 2 从配置文件加载ORB特征点有关的参数,并新建特征点提取器
    ...
    // 主特征提取器 mpORBextractorLeft (左目特征提取器,只有一个camera时默认认为是Left)
    // 根据读取的5个参数创建orb特征提取器
    // 无论哪种相机,tracking过程都会用到mpORBextractorLeft作为特征点提取器
    mpORBextractorLeft = new ORBextractor(  nFeatures,      //参数见上面的注释
                                            fScaleFactor,
                                            nLevels,
                                            fIniThFAST,
                                            fMinThFAST);

    // 右目特征提取器 mpORBextractorRight
    // 双目在tracking过程中还用到mpORBextractorRight作为右目特征点提取器
    ...
    // 单目初始化时的特征提取器 mpIniORBextractor
    if(sensor==System::MONOCULAR)
        mpIniORBextractor = new ORBextractor( 2*nFeatures,      // 注意提取的特征点数是其他提取器的两倍
                                                fScaleFactor,
                                                nLevels,
                                                fIniThFAST,
                                                fMinThFAST);
    ...
    {
        // mThDepth是一个阈值, 用于判断一个3D点为近点/远点
        // 近点特征点对三角化得到的三维点坐标比较准确，可以提供旋转、平移、尺度信息
        // 远点特征点仅能够提供相对准确的旋转信息，无法提供比较准确的尺度和平移信息, 只有当多个视角下 多次观测到的远点我们才会考虑三角化

        // ThDepth参数从RGB-D的yaml配置文件中读取到, ThDepth表示基线长度的多少倍
        // mbf = 基线长度 * 相机的焦距
        // mThDepth = 基线长度 * 相机的焦距 * 基线长度的倍数 / 相机的焦距 = 基线长度 * 基线长度的倍数 = 最终判断阈值
        mThDepth = mbf*(float)fSettings["ThDepth"]/fx;
        cout << endl << "Depth Threshold (Close/Far Points): " << mThDepth << endl; //输出mThDepth阈值
    }
...
//以下函数用于设置进程指针以关联进程, 在System函数中被调用
...
// 三种相机的Tracking: GrabImageStereo, GrabImageRGBD, GrabImageMonocular
    ...
    const double &timestamp)        //时间戳
{
    ...
    // step 2 ：将深度相机的disparity转为Depth, 也就是根据数据集中深度图的缩放, 转换成为真正尺度下的深度
...
// 1.将彩色图像转为灰度图像
// 2.构造Frame(根据是否是初始化选择用左目特征提取器/单目初始化特征提取器)
// 3.跟踪
    ...
    const double &timestamp)    //时间戳
{
    ...
    // Step 2 ：构造Frame
    // 需要判断该帧是不是初始化
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET) //没有成功初始化的前一个状态就是NO_IMAGES_YET, 见Track里的mState状态转换
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
            mpIniORBextractor,      //使用初始化ORB特征点提取器, 会提取2倍的指定特征点数目
            mpORBVocabulary,
            mK,
            mDistCoef,
            mbf,
        ...
        mCurrentFrame = Frame(
            mImGray,
            timestamp,
            mpORBextractorLeft,     //使用正常运行的时的左木ORB特征点提取器
            mpORBVocabulary,
            mK,
            mDistCoef,
            mbf,
    ...
    // 返回当前帧的位姿
    // 相机位姿: 世界坐标系到相机坐标坐标系的变换矩阵
    // cv::Mat类型做深拷贝
    return mCurrentFrame.mTcw.clone();
}


// 主tracking函数, 独立于传感器, 在每一个GrabImage函数内的最后运行
...
void Tracking::Track()
{
    ...
    // mState为tracking的状态, 状态包括 SYSTME_NOT_READY, NO_IMAGE_YET, NOT_INITIALIZED, OK, LOST
    // 如果图像复位过、或者第一次运行，则为图像序列的第一张图, 是NO_IMAGE_YET状态
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        //这个状态量在上面的初始化函数中被更新
        if(mState!=OK)
            return; //非正常tracking工作状态,退出(SYSTME_NOT_READY, NO_IMAGE_YET, NOT_INITIALIZED,LOST)
    }
    else
    {
        // 系统已成果初始化, 跟踪帧
        // bOK为临时变量，用于存储每个函数是否执行成功
        bool bOK;

        // mbOnlyTracking标记, 当前系统是处于SLAM状态还是纯定位状态
        // false->正常SLAM模式（定位+地图更新）, true->仅定位模式
        // tracking 类构造时, 默认为false
        // 在viewer中有个开关ActivateLocalizationMode，可以控制是否开启mbOnlyTracking
        if(!mbOnlyTracking)
        {
            // mbOnlyTracking == false时
            // 运行括号内, 做地图更新, 即启动Local Mapping
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Step 4 全局BA优化，同时优化所有位姿和三维点, 只有两帧这里
