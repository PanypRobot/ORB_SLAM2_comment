        			 ...
        			 mbDeactivateLocalizationMode(false)		//没有这个模式转换标志
{
    ...
    // 创建三大线程
    // Tracking线程是主线程, LocalMapping和LoopClosing是其子线程,所以用mptLocalMapping和mptLoopClosing控制子线程
    // 编程上构成父子线程关系, 实际上三个线程并行
    ...
    //创建 mptLocalMapping线程 和mpLocalMapper
    ...
    //创建 mptLoopClosing线程 和 mpLoopCloser
...
//终止局部建图和回环线程
//终止窗口查看器
//调用子线程的终止函数,以发送终止请求,使用usleep和while进行阻塞直到终止
