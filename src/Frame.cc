...
// 下一个生成的帧的ID
// 这里初始化的是 类的静态成员变量
// 静态成员变量为所有实例所共享, 而修改对于所有实例可见,
// 即所有帧维护一份独一的nNextId, 每次创建帧自增
long unsigned int Frame::nNextId=0;


// 是否要进行初始化操作的标志
// 这里给这个标志置位的操作是在最初系统开始加载到内存的时候进行的
// 由于下一帧是整个系统的第一帧，需要置位
bool Frame::mbInitialComputations=true;


// 这里所有都是static成员变量, 需要再声明一次
...
Frame::Frame(const Frame &frame)
    :mpORBvocabulary(frame.mpORBvocabulary), 
     ...
     N(frame.N), 
     mvKeys(frame.mvKeys),									//单层vector的拷贝构造. 经实验, 确定这种通过同类型对象初始化的操作是具有深拷贝的效果的
...
{
    // 对于单层vector的拷贝构造, 可以直接在参数列表指明
	// 对于双层嵌套的vector类型的深拷贝, 应该没有更简便的方法做深拷贝
    for(int i=0;i<FRAME_GRID_COLS;i++)  // 网格列, 64
        for(int j=0; j<FRAME_GRID_ROWS; j++)    // 网格行, 48
            mGrid[i][j]=frame.mGrid[i][j]; // 逐个拷贝

    // 给新的帧设置Pose
    // ??这里判断是否为空, 是否是作为拷贝构造目标的帧可能已经是一个构造好的对象, 已经具有Pose??
    if(!frame.mTcw.empty())
        SetPose(frame.mTcw);
}
...
Frame::Frame(   const cv::Mat &imLeft,          //左目图像
                ...
                ORBVocabulary* voc,             //词袋 orb特征字典
                cv::Mat &K,                     //内参矩阵
                cv::Mat &distCoef,              //畸变矩阵
                const float &bf,                //基线*焦距
...
// RGBD帧 构造函数
Frame::Frame(   const cv::Mat &imGray,      //RGB灰度化的灰度图像
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    if(flag==0) // 0左图
        ...
        (*mpORBextractorLeft)(  im,				//待提取特征点的图像
							    cv::Mat(),		//掩摸图像, 实际没有用到
							    mvKeys,			//输出变量，保存 原始左图像提取出的特征点
							    mDescriptors);	//输出变量，保存 左目特征点对应的描述子
    else    // 1右图
        // 右图指定的特征点提取器
        (*mpORBextractorRight)( im,
                                cv::Mat(),
                                mvKeysRight,        //输出变量，保存 原始右图像提取出的特征点
                                mDescriptorsRight); //输出变量，保存 右目特征点对应的描述子
}

// 设置相机姿态
void Frame::SetPose(cv::Mat Tcw)
{
    mTcw = Tcw.clone();     // 将位姿成员变量设置为Tcw
    UpdatePoseMatrices();   // 更新其他位姿相关成员变量
}

// 根据Tcw
// 计算更新mRcw mtcw mRwc mOw
// 被SetPose调用
void Frame::UpdatePoseMatrices()
{
	...
	// 从变换矩阵中提取出 旋转矩阵
    // rowRange这个只取到范围的左边界，而不取右边界, 所以这里是左上角开始取的3*3
    ...
    // mtcw 从世界坐标系变换到相机坐标系, 然后再将其取反, 最后再将其从相机坐标系变换到世界坐标系, 得到的就是相机光心在世界坐标系下的坐标 mOw
    // ??这里还是有点不明白??
    mOw = -mRcw.t()*mtcw;
}
...
{
    // Step 1 若第一个畸变参数为0, 不需要矫正
    // 第一个畸变参数k1是最重要的, 一般不为0, 为0的话, 说明畸变参数都是0
	// 变量mDistCoef中存储了opencv指定格式的去畸变参数, 格式为：(k1,k2,p1,p2,k3)
    if(mDistCoef.at<float>(0)==0.0) //第一个畸变参数为0, 不需要校正
    {
        mvKeysUn=mvKeys;    //不需要校正, 直接把mvKeys赋给mvKeysUn
        return;
    }
    ...
    // N是成员变量, 是提取的特征点数量
    // 将N个特征点保存在矩阵中, 后进行reshape, 以满足OpenCV函数输入要求
    cv::Mat mat(N,2,CV_32F); // N行 2列 1通道
	// 遍历每个特征点，并将它们的坐标保存到矩阵中. 第一个维度为特征点索引, 第二个分别为0横坐标1纵坐标
    for(int i=0; i<N; i++)
    {
        mat.at<float>(i,0)=mvKeys[i].pt.x;  // 横坐标保存
        mat.at<float>(i,1)=mvKeys[i].pt.y;  // 纵坐标保存
    }
    ...
    mat=mat.reshape(2); // 通道变为2, 行数不变, N行 1列 2通道
    cv::undistortPoints(	
		mat,				//输入的特征点坐标
	...
	//调整回只有一个通道，回归正常处理方式
    mat=mat.reshape(1); // 通道变为1, 行数不变, N行 2列 1通道


    // Step 4 存储校正后的特征点
    mvKeysUn.resize(N); // size可容纳N个元素
	//遍历每一个特征点
    for(int i=0; i<N; i++)
    {
		//根据索引获取对应的特征点的副本
		//之所以这样做而不是直接重新声明一个特征点对象, 目的是能够得到源特征点对象的其他属性, 而只更改坐标
        cv::KeyPoint kp = mvKeys[i];
		//读取校正后的坐标 覆盖旧坐标
        kp.pt.x=mat.at<float>(i,0); //更改x
        kp.pt.y=mat.at<float>(i,1); //更改y
        mvKeysUn[i]=kp; //把对应修改过坐标的mvKeys副本赋值给mvKeysUn
    }
}
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft)	
{
    ...
    if(mDistCoef.at<float>(0)!=0.0)
	{
        ...
        mat.at<float>(0,0)=0.0;         //左上 (0,0)
		mat.at<float>(0,1)=0.0;
        mat.at<float>(1,0)=imLeft.cols; //右上 (cols,0)
		mat.at<float>(1,1)=0.0;
		mat.at<float>(2,0)=0.0;         //左下 (0,rows)
		mat.at<float>(2,1)=imLeft.rows;
        mat.at<float>(3,0)=imLeft.cols; //右下 (cols,rows)
		mat.at<float>(3,1)=imLeft.rows;

        // 边界点校正操作, 和前面校正特征点一样的操作
        mat=mat.reshape(2);
        cv::undistortPoints(mat,mat,mK,mDistCoef,cv::Mat(),mK);
        mat=mat.reshape(1);

		// 校正后的四个边界点已不能够围成一个严格的矩形
        // 因此在这个四边形的外侧加边框作为坐标的边界
...
void Frame::ComputeStereoMatches()
{
    ...
    // 定义两个vector用于存储匹配结果
    // 预先分配了内存, 数据类型为float型, 大小为N, 数据初始值为-1.0f
    // mvuRight 存储右图匹配点索引
    // mvDepth  存储特征点的深度信息
	...
	// orb特征相似度阈值
    // thOrbDist = (TH_HIGH  + TH_LOW) / 2, 高阈值和低阈值都在ORBmatcher中定义
    const int thOrbDist = (ORBmatcher::TH_HIGH+ORBmatcher::TH_LOW)/2;

    // 金字塔底层（0层）图像的行数(高度) nRows
    // mvImagePyramid为cv::Mat, rows为其成员变量, 表示行数
    const int nRows = mpORBextractorLeft->mvImagePyramid[0].rows;

	// 二维vector存储每一行的orb特征点的列坐标的索引
    // vector确保可扩展, 每一行的特征点数量不一样
    // size_t表示非负整数, 以确保移植性
    ...
    // vRowIndices 初始化为大小为图像行数, 成员为 vector<size_t>() 的一个变量
    for(int i=0; i<nRows; i++) 
        // 为每行的vector预留200, 用于存储列号. 因为每行特征点不太肯呢个超过200
	...
	// Step 1. 行特征点统计, 目标为构建vRowIndices
    // 考虑用图像金字塔尺度作为偏移, 左图中对应右图的一个特征点可能存在于多行, 而非唯一的一行
    for(int iR = 0; iR < Nr; iR++) {    //遍历每个特征点, iR为特征点序号

        // 获取kp特征点ir的y坐标, 即行号
        const cv::KeyPoint &kp = mvKeysRight[iR];
        const float &kpY = kp.pt.y;
        
        // 计算特征点ir在行方向上大致的区域. r为可能的偏移范围, 即可能的行号为[kpY + r, kpY -r]
        // 2 表示在全尺寸(scale = 1)的情况下，假设有2个像素的偏移，随着尺度变化，r也跟着变化
        // ??不知道r这个值怎么得到??
        // octave为cv::KeyPoint的成员, 表示关键点所在的尺度空间层数, 在ComputeKeyPointsOctTree函数中被赋值, 记录了检测到该特征点的层数
        const float r = 2.0f * mvScaleFactors[mvKeysRight[iR].octave];
        const int maxr = ceil(kpY + r);     // 进一法取整
        const int minr = floor(kpY - r);    // 去尾法取整

        // 将特征点ir保证在可能的行号中
        // 事实上在金字塔高层级上的特征点对应好几行, 先存储, 后会做精匹配
        for(int yi=minr;yi<=maxr;yi++)
            vRowIndices[yi].push_back(iR); //将特征点序号给到vRowIndices[]可能得对应行中
    }

    // 下面是 粗匹配 + 精匹配
    // 对于立体矫正后的两张图，在列方向(x)存在最大视差maxd和最小视差mind
    // 即对于左图中任一点p，在右图上的匹配点的列坐标范围为应该是[p - maxd, p - mind], 而不需要遍历每一行所有的像素

    // maxD = baseline * length_focal / minZ,
    // minZ为mb(baseline), 对应maxd = f
    // minD = baseline * length_focal / maxZ,
    // maxZ为无穷远, 对应minD = 0

    const float minZ = mb;          // minZ = mb = baseline, maxZ = 无穷远处
    const float minD = 0;
    const float maxD = mbf/minZ;

    // pair保存 sad块匹配相似度, 左图特征点索引
    vector<pair<int, int> > vDistIdx;
    vDistIdx.reserve(N);    //预留大小为特征点数量

    // 遍历左图每一个特征点il, 在右图搜索最相似的特征点ir
    for(int iL=0; iL<N; iL++) { //遍历左图特征点索引

		const cv::KeyPoint &kpL = mvKeys[iL];   // 使用索引从mvKeys中取出特征点
        const int &levelL = kpL.octave;         // 每个特征点的层级, 用levelL引用
        const float &vL = kpL.pt.y;             // vL引用, 每个特征点的纵坐标
        const float &uL = kpL.pt.x;             // uL引用, 每个特征点的横坐标
