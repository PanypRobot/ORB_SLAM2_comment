...
/**
 * @brief 地图, 本质是维护两个set: mspMapPoints, mspKeyFrames
 * 
 */
class Map
{
public:
    ...
    //互斥锁
    //互斥量: 当更新地图时.回环检测中和局部BA后更新全局地图的时候会用到这个
    std::mutex mMutexMapUpdate;
    //互斥量: 避免不同线程同时创建地图点, 导致id冲突设计
...
protected:
    ...
    // 貌似在程序中并没有被使用过
    // 与地图中的大变化相关的索引 (loop closure, global BA)
    int mnBigChangeIdx;

    //类的成员函数在操作类成员变量时,防止冲突的互斥量
    std::mutex mMutexMap;
};
